...
Client.prototype.connect = function(options, callback) {
  ...
  function onpacket(buf) {
    ...
    debug(
      'recv message: host=%s protocolVersion=%s sourceId=%s destinationId=%s namespace=%s data=%s',
      self.host,
    ...
    if(message.protocolVersion !== 0) { // CASTV2_1_0
      ...
      self.close();
      debug('Unsupported protocol version: ' + message.protocolVersion);
      return;
    }
...
Client.prototype.send = function(sourceId, destinationId, namespace, data) {
  ...
  debug(
    'send message: host=%s protocolVersion=%s sourceId=%s destinationId=%s namespace=%s data=%s',
    this.host,
    message.protocolVersion,
    message.sourceId,
    message.destinationId,
  ...
  if(this.ps) {
    ...
    debug('Trying to emmit on a closed connection.');
    this.emit('close');
  }
};
